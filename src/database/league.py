...
class LeagueDatabase:
    ...
    def update_league(self, league_id: str) -> Optional[pd.DataFrame]:
        ...
        season = int(history_df.iloc[0]['Season'])
        update_df = self._download_league(league=league, start_year=season)

        if update_df is not None:
            # football-data occasionally publishes a wrong competition under a
            # season URL. Reject additions whose clubs have no meaningful
            # overlap with the latest known season (promoted clubs are allowed).
            history_keys = set(zip(
                history_df['Date'].astype(str), history_df['Home'], history_df['Away'],
            ))
            additions = update_df[[
                tuple(row) not in history_keys
                for row in zip(update_df['Date'].astype(str), update_df['Home'], update_df['Away'])
            ]]
            if not additions.empty:
                latest_season = int(history_df['Season'].max())
                known_rows = history_df[history_df['Season'] == latest_season]
                known_teams = set(known_rows['Home']) | set(known_rows['Away'])
                added_teams = set(additions['Home']) | set(additions['Away'])
                overlap = len(known_teams & added_teams) / max(1, len(added_teams))
                if overlap < 0.40:
                    raise ValueError(
                        f'{league_id} 下载数据疑似来自错误联赛：'
                        f'新球队与历史球队重叠率仅 {overlap:.0%}，已拒绝写入。'
                    )
