...
def _settled_record(prediction: pd.Series, result: dict) -> Optional[dict]:
    ...
    return {
        ...
        'handicap_probability': _float(prediction, '让球首选概率'),
        'handicap_odds_home': _float(prediction, '官方让胜奖金'),
        'handicap_odds_draw': _float(prediction, '官方让平奖金'),
        'handicap_odds_away': _float(prediction, '官方让负奖金'),
        ...
        'half_model_source': str(prediction.get('半场模型来源') or ''),
        'half_p_home': _float(prediction, '正式半场胜概率'),
        'half_p_draw': _float(prediction, '正式半场平概率'),
        'half_p_away': _float(prediction, '正式半场负概率'),
        **monte_carlo_fields,
