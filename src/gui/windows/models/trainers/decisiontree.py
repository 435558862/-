...
from PySide6.QtWidgets import QComboBox, QHBoxLayout, QSlider, QVBoxLayout
...
class DecisionTreeTrainerDialog(TrainerDialog):
    ...
    def _add_trainer_widgets(self, root: QVBoxLayout):
        ...
        for criterion in self._criterion_options:
            self._combo_criterion.addItem(option_zh(criterion), criterion)
        ...
        for feature in self._feature_options:
            self._combo_features.addItem(option_zh(feature), feature)
        self._add_tunable_param(
            name='Max Features',
        ...
        for class_weight in self._class_weights:
            self._combo_class.addItem(option_zh(class_weight), class_weight)
        self._add_tunable_param(
            name='Class Weight',
    ...
    def _get_model_params(self) -> Dict[str, Any]:
        return {
            'criterion': self._criterion_options[self._combo_criterion.currentData()],
            'min_samples_leaf': self._slider_leaf.value(),
            'min_samples_split': self._slider_samples.value(),
            'max_features': self._feature_options[self._combo_features.currentData()],
