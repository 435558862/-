...
from PySide6.QtWidgets import QComboBox, QHBoxLayout, QSlider, QSpinBox, QDoubleSpinBox, QVBoxLayout
...
class NeuralNetworkTrainerDialog(TrainerDialog):
    ...
    def _add_trainer_widgets(self, root: QVBoxLayout):
        ...
        for activation in self._activations:
            self._combo_hidden_activation.addItem(option_zh(activation), activation)
        ...
        for vsn_option in self._vsns:
            self._combo_vsn.addItem(option_zh(vsn_option), vsn_option)
        self._add_tunable_param(
        ...
        for layer_norm in self._layer_normalizations:
            self._combo_layer_normalization.addItem(option_zh(layer_norm), layer_norm)
        self._add_tunable_param(
        ...
        for batch_norm in self._batch_normalizations:
            self._combo_batch_normalization.addItem(option_zh(batch_norm), batch_norm)
        self._add_tunable_param(
        ...
        for weight in self._class_weights:
            self._combo_class.addItem(option_zh(weight), weight)
        self._add_tunable_param(
            name='Class Weight',
        ...
        for optim in self._optimizers:
            self._combo_optimizer.addItem(option_zh(optim), optim)
        self._add_tunable_param(
            name='Optimizer',
        ...
        for weight in self._class_weights:
            self._combo_lookahead.addItem(option_zh(weight), weight)
        self._add_tunable_param(
            name='Lookahead',
    ...
    def _get_model_params(self) -> Dict[str, Any]:
        return {
            ...
            'hidden_activation': self._activations[self._combo_hidden_activation.currentData()],
            'vsn': self._vsns[self._combo_vsn.currentData()],
            'layer_normalization': self._layer_normalizations[self._combo_layer_normalization.currentData()],
            'batch_normalization': self._batch_normalizations[self._combo_batch_normalization.currentData()],
            'dropout_rate': self._slider_dropout.value(),
            'odd_noise_std': self._slider_noise.value(),
            'class_weight': self._class_weights[self._combo_class.currentData()],
            'optimizer': self._optimizers[self._combo_optimizer.currentData()],
            'lookahead': self._lookaheads[self._combo_lookahead.currentData()],
