...
class DiscriminantTrainerDialog(TrainerDialog):
    ...
    def _add_trainer_widgets(self, root: QVBoxLayout):
        ...
        for oas in self._oas_options:
            self._combo_oas.addItem(option_zh(oas), oas)
        ...
        for boundary in self._boundaries:
            self._combo_boundaries.addItem(option_zh(boundary), boundary)
        self._add_tunable_param(
    ...
    def _get_model_params(self) -> Dict[str, Any]:
        return {
            'oas': self._oas_options[self._combo_oas.currentData()],
            'decision_boundary': self._boundaries[self._combo_boundaries.currentData()]
        }
