...
class KNNTrainerDialog(TrainerDialog):
    ...
    def _add_trainer_widgets(self, root: QVBoxLayout):
        ...
        for weight in self._weights:
            self._combo_weights.addItem(option_zh(weight), weight)
        ...
        for metric in self._distances:
            self._combo_distances.addItem(option_zh(metric), metric)
    ...
    def _get_model_params(self) -> Dict[str, Any]:
        return {
            ...
            'weights': self._weights[self._combo_weights.currentData()],
            'p': self._distances[self._combo_distances.currentData()]
        }
