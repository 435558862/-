...
class TrainerDialog(QDialog):
    ...
    def __init__(
        ...
        self._target_types = {
            ...
            'U/O-2.5': TargetType.OVER_UNDER,
            'Half/Full Result': TargetType.HALF_FULL,
            'Exact Score': TargetType.SCORE,
            'Half-time Result': TargetType.HALF_RESULT,
        }
    ...
    def get_model_params(self, model_id: str) -> Dict[str, Any]:
        ...
        model_params = {
            ...
            'normalizer': self._normalizer_types[self._combo_norm.currentData()],
            'sampler': self._sampler_types[self._combo_sampler.currentData()]
        }

        if self._supports_calibration:
            model_params['calibrate_probabilities'] = self._calibration_options[self._combo_calibration.currentData()]
    ...
    def train(self):
        ...
        runner = TaskRunnerDialog(
            ...
            info='正在后台训练模型，请稍候…',
            task_fn=lambda: self._trainer.train(
                model=model,
                train_df=train_df,
                eval_df=eval_df,
                check_nan=True,
        ...
        if runner.error_message is not None or result is None:
            QMessageBox.critical(
                self,
                'Model Creation Failed',
                f'模型训练失败：{runner.error_message or "没有返回训练结果"}',
            )
            return
        model, metrics_df = result
        metrics_df['Model'] = model_id
        metrics_df['Model Type'] = model.__class__
    ...
    def tune(self, model_cls: Type, fixed_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metric = self._combo_objective.currentData()
    ...
    def _add_widgets(self):
        ...
        for target in self._target_types:
            self._combo_target.addItem(option_zh(target), target)
        ...
        for objective in self._optimization_metrics:
            self._combo_objective.addItem(option_zh(objective), objective)
        ...
        for normalizer in self._normalizer_types:
            self._combo_norm.addItem(option_zh(normalizer), normalizer)
        ...
        for sampler in self._sampler_types:
            self._combo_sampler.addItem(option_zh(sampler), sampler)
        self._add_tunable_param(
        ...
        if self._supports_calibration:
            ...
            for calibration in self._calibration_options:
                self._combo_calibration.addItem(option_zh(calibration), calibration)
            self._add_tunable_param(
        ...
        self._check_cross_valid.setChecked(False)
        ...
        self._check_sliding_cross_valid.setChecked(False)
