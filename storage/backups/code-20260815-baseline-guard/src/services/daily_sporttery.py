...
# Chronological audit: 4,076 matches dated 2025-01-01 or later from the nine
# local leagues. These are selection thresholds, not a promise for one match.
MARKET_SELECTION_PROFILE = (
    (0.70, '精选主推', 0.8081841432, 0.0959273798),
    (0.60, '高置信主推', 0.7241746539, 0.2303729146),
    (0.55, '谨慎主推', 0.6884384384, 0.3267909715),
    (0.50, '观察', 0.6382047072, 0.4482335623),
    (0.00, '跳过', 0.5252698724, 1.0),
)
...
def _clubelo_ratings(as_of: str) -> dict[str, float]:
    ...
    if not cache_path.exists():
        ...
        try:
            ...
            logging.exception('ClubElo 获取失败：%s', as_of)
...
def _market_baseline_probabilities(odds: Dict[str, float]) -> dict:
...
def _score_ranking_consistent_with_total(
    ...
    Over/under is an aggregate over many scores, while an exact score is one
    cell. Restricting the displayed score shortlist prevents confusing output
    such as "over 2.5" beside "2-0" without altering either model's raw
    probabilities.
    """
    totals = np.array([
